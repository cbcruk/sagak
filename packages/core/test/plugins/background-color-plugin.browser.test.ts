...
} from '@/plugins/background-color-plugin'
...
describe('BackgroundColorPlugin (배경 색상 설정)', () => {
  ...
  describe('플러그인 등록 (기본 초기화)', () => {
    /**
     * Why: `BackgroundColorPlugin`을 `PluginManager`에 등록하여 사용 가능한 상태로 만들어야 합니다
     * How: `PluginManager.register()`를 호출하고 `has()`로 등록 여부를 확인합니다
     */

    it('BackgroundColorPlugin을 등록해야 함', async () => {
      // Given: 빈 PluginManager

      // When: BackgroundColorPlugin 등록
      await pluginManager.register(BackgroundColorPlugin)

      // Then: 플러그인이 'text-style:background-color' 이름으로 등록됨
    ...
    it('커스텀 옵션으로 플러그인을 생성해야 함', async () => {
      // Given: 커스텀 옵션 설정
      const customPlugin = createBackgroundColorPlugin({
        eventName: 'CUSTOM_BG_COLOR',
  ...
  describe('배경 색상 명령 실행 (backColor 설정)', () => {
    /**
     * Why: 사용자가 선택한 텍스트에 배경 색상을 적용해야 합니다
     * How: `EventBus`로 `BACKGROUND_COLOR_CHANGED` 이벤트를 발생시키고, 내부적으로 `document.execCommand('backColor')`를 실행합니다
     */

    beforeEach(async () => {
      // Given: BackgroundColorPlugin 등록
      await pluginManager.register(BackgroundColorPlugin)
    })
    ...
    it('색상 변경 성공 후 STYLE_CHANGED 이벤트를 발생시켜야 함', () => {
      ...
      // When: BACKGROUND_COLOR_CHANGED 이벤트 발생
  ...
  describe('색상 형식 검증 (유효한 색상 확인)', () => {
    ...
    it('유효한 hex 색상(#RGB)을 허용해야 함', () => {
      ...
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: '#0FF' })
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: '#F0F' })
    ...
    it('유효한 rgb 색상을 허용해야 함', () => {
      ...
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: 'rgb(0,255,255)' })
    ...
    it('색상 이름을 허용해야 함', () => {
      ...
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: 'cyan' })
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: 'magenta' })
    ...
    it('유효하지 않은 색상 형식을 거부해야 함', () => {
      ...
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: 'rgb(300, 0, 0)' }) // Still passes regex but invalid value
    ...
    it('validateFormat이 false일 때 검증을 건너뛰어야 함', async () => {
      ...
      eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: 'any-value' })
  ...
  describe('허용된 색상 검증 (화이트리스트 확인)', () => {
    ...
    it('허용 목록에 없는 색상을 차단해야 함', async () => {
      ...
      expect(consoleWarn).toHaveBeenCalledWith(
        'Background color blocked: "#FF00FF" is not in allowed colors'
      )
  ...
  describe('검증 (입력값 확인)', () => {
    ...
    it('색상이 제공되지 않으면 차단해야 함', () => {
      ...
      expect(result).toBe(false)
      expect(consoleWarn).toHaveBeenCalledWith(
        'Background color blocked: No color provided'
      )
      expect(execCommandSpy).not.toHaveBeenCalled()
  ...
  describe('플러그인 생명주기 (초기화/정리)', () => {
    ...
    it('destroy 시 정리를 수행해야 함', async () => {
      ...
      let result = eventBus.emit('BACKGROUND_COLOR_CHANGED', {
        color: '#FFFF00',
      })
      expect(result).toBe(true)
      ...
      result = eventBus.emit('BACKGROUND_COLOR_CHANGED', { color: '#FFFF00' })
  ...
  describe('커스텀 이벤트 이름 (이벤트 설정)', () => {
     ...
     * How: `createBackgroundColorPlugin({ eventName })` 옵션으로 커스텀 이벤트 이름을 설정합니다
     */

    it('커스텀 이벤트 이름을 수신해야 함', async () => {
      // Given: 커스텀 이벤트 이름을 가진 플러그인
      const customPlugin = createBackgroundColorPlugin({
        eventName: 'MY_BG_COLOR_EVENT',
      })
      ...
      // When: 커스텀 이벤트 발생
      const result = eventBus.emit('MY_BG_COLOR_EVENT', { color: '#FFFF00' })
