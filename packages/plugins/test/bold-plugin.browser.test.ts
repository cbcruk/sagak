...
import { createBoldPlugin, BoldPlugin } from '../src/bold-plugin'
...
describe('BoldPlugin (굵게 텍스트 스타일 적용)', () => {
  ...
  describe('플러그인 등록 (기본 초기화)', () => {
    ...
    it('커스텀 옵션으로 플러그인을 생성해야 함', async () => {
      ...
      const customPlugin = createBoldPlugin({
        eventName: 'CUSTOM_BOLD',
  ...
  describe('굵게 명령 실행 (execCommand 호출)', () => {
    /**
     * Why: 사용자가 굵게 버튼 클릭 시 선택된 텍스트에 굵게 스타일 적용
     * How: `BOLD_CLICKED` 이벤트 수신 → `execCommand('bold')` 실행
     */
    ...
    it('굵게 성공 후 STYLE_CHANGED 이벤트를 발생시켜야 함', () => {
      ...
      expect(styleChangedSpy).toHaveBeenCalledWith({ style: 'bold' })
  ...
  describe('이벤트 단계 (3단계 실행 흐름)', () => {
    ...
    it('3단계(BEFORE/ON/AFTER)를 모두 실행해야 함', () => {
      ...
      eventBus.on('BOLD_CLICKED', 'before', beforeSpy)
      eventBus.on('BOLD_CLICKED', 'on', onSpy)
      eventBus.on('BOLD_CLICKED', 'after', afterSpy)
  ...
  describe('커스텀 이벤트 이름 (이벤트 설정)', () => {
     ...
     * How: `createBoldPlugin({ eventName })` 옵션으로 커스텀 이벤트 설정
     */

    it('커스텀 이벤트 이름을 수신해야 함', async () => {
      // Given: 커스텀 이벤트 이름이 설정된 플러그인
      const customPlugin = createBoldPlugin({
        eventName: 'MY_BOLD_EVENT',
      })
      await pluginManager.register(customPlugin)
      ...
      // When: 커스텀 이벤트 발생
      const result = eventBus.emit('MY_BOLD_EVENT')
  ...
  describe('에러 처리 (예외 상황 대응)', () => {
    ...
    it('execCommand 에러를 안전하게 처리해야 함', () => {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        'Failed to execute bold command:',
        expect.any(Error)
      )
  ...
  describe('플러그인 생명주기 (초기화/정리)', () => {
    ...
    it('destroy 시 정리를 수행해야 함', async () => {
      ...
      result = eventBus.emit('BOLD_CLICKED')
      expect(execCommandSpy).not.toHaveBeenCalled()
    ...
    it('destroy 후 재등록을 허용해야 함', async () => {
      ...
      // When: 재등록 시도
      await expect(newManager.register(BoldPlugin)).resolves.not.toThrow()

      // Then: 정상적으로 등록됨
      expect(newManager.has('text-style:bold')).toBe(true)
