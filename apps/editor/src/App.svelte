<script lang="ts">
  import Editor from 'sagak-ui/svelte/Editor.svelte'
  import Toolbar from 'sagak-ui/svelte/Toolbar.svelte'
  import DocumentBar from 'sagak-ui/svelte/DocumentBar.svelte'
  import AutocompletePopover from 'sagak-ui/svelte/AutocompletePopover.svelte'
...
<main data-scope="app">
  ...
  {#if error}
    <p role="alert" data-part="error">에디터를 시작하지 못했습니다: {error.message}</p>
  {/if}

  <Editor initialContent={INITIAL_CONTENT} onfail={(e) => (error = e)}>
