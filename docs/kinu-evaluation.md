...
> **정정 (§12 참고).** 위 진단은 절반이 틀렸습니다. 커서를 실제로 목록 안에 두면
> 트리거는 정상 동작합니다 — 앞선 확인에서 클릭이 목록 밖에 떨어진 것을 잘못 읽었습니다.
> 진짜 문제는 더 넓었고, 원인도 `getCurrentListType()` 이 아니었습니다.

## 12. 툴바를 쓴 뒤 타이핑이 사라지던 문제

§11 에서 "목록 트리거 상태" 로 적어둔 것을 파고들다 더 큰 것이 나왔습니다.

### 실제 증상

**툴바 버튼을 누른 뒤 이어서 타이핑하면 키 입력이 사라집니다.** 목록만이 아니라
정렬·굵게 등 툴바 버튼 전부에 해당했습니다.

버튼을 누르면 포커스가 그 버튼으로 옮겨갑니다. 커맨드 자체는 저장된 선택 영역으로
동작하므로 서식은 제대로 적용되지만, 편집 영역은 포커스를 잃은 상태로 남습니다.
그 뒤의 키 입력은 갈 곳이 없습니다.

`#15` 이전 커밋(`1eb8d76`)을 빌드해 같은 절차를 돌려 **kinu 이전과 무관한
기존 문제임을 확인**했습니다. 직접 만든 팝오버 시절에는 포커스가 `BODY` 로,
kinu `DropdownMenu` 에서는 트리거 버튼으로 갈 뿐 결과는 같았습니다.

### 앞선 진단이 틀린 이유

§11 을 쓸 때 "목록 안을 클릭했는데도 트리거가 안 켜진다" 고 적었는데, 그 확인에서
Playwright 의 클릭이 목록 밖(컨테이너 여백)에 떨어져 있었습니다. 커서를 `Ctrl+Home`
으로 옮기거나 항목 텍스트를 좌표로 정확히 클릭하면 트리거는 `on` 으로 바뀝니다.
`getCurrentListType()` 은 멀쩡했습니다.

목록에서만 눈에 띈 이유는, 목록이 "만들고 바로 이어서 쓰는" 동작이기 때문입니다.
정렬이나 굵게는 이미 쓴 글에 적용하는 경우가 많아 같은 문제가 가려져 있었습니다.

### 고친 방법

커맨드가 성공한 뒤 편집 영역으로 포커스를 되돌립니다. 이건 UI 의 관심사가 아니라
에디터 코어의 규약이므로, 모든 커맨드가 지나는 `runCommand` 한 곳에 담았습니다.
바로 위에 이미 같은 모양의 규약(`CAPTURE_SNAPSHOT`)이 있어 그 자리를 따랐습니다.

```ts
export function runCommand(registry, eventBus, name, value): boolean {
...
`EditorCore` 가 `FOCUS_REQUESTED` 를 구독해 `focus()` 를 호출합니다. 툴바·단축키·
프로그램 호출 어느 경로로 들어와도 같게 동작합니다.

성공한 커맨드에서만 발행합니다. 실패한 커맨드까지 포커스를 옮기면 사용자가
의도적으로 다른 곳에 둔 포커스를 빼앗게 됩니다.

### 검증

포커스 복귀 **10/10 통과**. 목록 생성 뒤 이어서 친 글자가 실제로 `<li>` 안으로
들어가는지(`<li>list meXYZ</li>`), 정렬·굵게 뒤에도 포커스가 돌아오는지,
그러면서 **셀렉트와 다이얼로그는 포커스를 빼앗기지 않는지**까지 확인했습니다.
셀렉트는 포커스가 그대로여야 드롭다운이 유지되고, 다이얼로그는 열린 채로 남아야 합니다.

기존 기능 33/33 · 테마 26/26 도 그대로 통과합니다.

단위 테스트 5개를 더했습니다 — `runCommand` 가 성공 시 실행 뒤에 발행하는지,
...
이번 세션 내내 브라우저를 직접 몰아 버그를 찾았습니다 — 다크 모드 분열,
다이얼로그가 안 닫힘, 툴바를 쓴 뒤 타이핑 유실, CSS 변수 자기참조.
**그 검증들은 전부 `/tmp` 의 일회용 Playwright 스크립트였습니다.** 69개 검사가
매번 버려졌고, CI 는 한 번도 본 적이 없습니다.

### 이미 있던 것

새로 도입할 것은 없었습니다. vitest 브라우저 모드가 Playwright provider 로
실제 Chromium 에서 돌고 있고, CI 도 `playwright install` 후 `pnpm test:run` 을
실행합니다. 문제는 **불균형**이었습니다.

| | 이전 | 이후 |
|---|---|---|
| `packages/core` | 48 파일 / 1014개 | 그대로 |
| `packages/ui` | 1 파일 / **4개** | 4 파일 / **35개** |

`packages/ui` 의 4개는 전부 font-family 와 eventBus 에 관한 것이었습니다.
이번 세션의 UI 버그가 하나도 걸리지 않은 이유입니다.

### 컴포넌트가 아니라 앱 구성을 띄웁니다

찾았던 버그들은 **컴포넌트를 따로 마운트해서는 보이지 않습니다.** 툴바와 편집
영역이 같은 에디터 컨텍스트로 이어져야 드러납니다. 그래서 `test/harness.tsx` 가
`apps/editor` 의 트리를 그대로 재현합니다.

옮기는 과정에서 설정 버그를 하나 찾았습니다. `vitest.config.ts` 에
`resolve.dedupe` 가 없어 **preact 인스턴스가 둘로 갈려 있었습니다** — 훅이
렌더 컨텍스트를 못 찾고 `Cannot read properties of undefined (reading '__H')` 로
죽습니다. 컴포넌트를 렌더하는 테스트가 하나도 없어서 지금까지 드러나지
않았습니다.

### 다크 모드는 CDP 로

`prefers-color-scheme` 을 진짜로 바꿔야 의미가 있습니다. vitest 브라우저
컨텍스트의 `cdp()` 로 `Emulation.setEmulatedMedia` 를 보냅니다. kinu 가 지원하는
`.dark` 클래스를 붙이는 우회도 가능하지만, 그건 실제 미디어 쿼리 경로를
건너뜁니다.

### 통과가 아니라 잡는지를 확인했습니다

§12 에서 적었듯 **검사가 통과했다는 것과 옳다는 것은 다릅니다.** 그래서 각
수정을 되돌려 해당 테스트가 실제로 실패하는지 확인했습니다.

| 되돌린 것 | 실패한 테스트 |
|---|---|
| `runCommand` 의 `FOCUS_REQUESTED` 발행 (§12) | 1 |
| `--sagak-accent: var(--sagak-accent)` 자기참조 (§11) | 3 |
| 종이 배경을 존재하지 않는 `[data-part='root']` 로 (§11) | 1 |
| 아이콘 버튼 배경을 `#fff` 로 고정 (§11 원본 증상) | 2 |

`#15` 의 대비 검사가 투명을 검정으로 취급해 21:1 로 **잘못 통과**했던 것도
`visibleBackground()` 로 조상 색을 합성해 막았습니다.

### 남은 것

Storybook 이 담던 시나리오는 아직 덮지 못했습니다 — 모바일·태블릿 뷰포트,
맞춤법 검사 on/off, 표 리사이즈, 자동 저장, 자동 완성. 이번에는 **실제 버그를
잡은 검증만** 옮겼고, 나머지는 해당 기능의 동작을 먼저 파악해야 합니다.

## 15. 문서 줄에 `Menubar` 넣기

문서 줄(`document-bar`)의 동작 다섯 — New · Documents… · Save · Save As… ·
Save to Computer… — 을 손으로 만든 `<button>` 에서 kinu 의 `Menubar` /
`MenubarItem` 으로 옮겼습니다.

### 모양만 넘겼습니다

`Menubar` 는 `<nav k="menubar">` 하나에 CSS 세 줄(flex · 항목 여백 · hover)
입니다. §10 의 `Select` 처럼 코드가 크게 주는 종류가 아니고, 실제로 준 것은
**한 줄로 읽히는 메뉴**입니다. 테두리 다섯 개가 사라지고 경계는 hover 에서만
드러납니다.

`MenubarItem` 이 갖고 있는 유일한 동작은 "이미 열린 드롭다운이 있을 때
`mouseenter` 로 건너뛰기" 인데, **지금은 드롭다운이 없으므로 아무 일도 하지
않습니다.** 얻은 것은 없지만 잃은 것도 없습니다.

### 드롭다운으로 접지 않은 이유

`File ▾` 안에 다섯 개를 넣는 쪽이 더 메뉴 바처럼 보입니다. 그러면 **저장이 두
번 클릭**이 됩니다. 자동 저장이 없는 에디터에서 가장 자주 누르는 것이 저장인데
(`docs/document-model.md`), 모양을 위해 그걸 한 겹 안으로 넣는 것은 방향이
반대입니다. 나중에 항목이 늘어 한 줄에 안 들어가면 그때 다시 볼 일입니다.

### 옆으로 새지 않게 한 것

`[data-scope='document-bar'] button` 규칙이 **문서 목록 다이얼로그의 버튼까지**
잡고 있었습니다 — `<dialog>` 가 문서 줄 안에 렌더되기 때문입니다. 메뉴 항목만
새 규칙으로 빼고 나머지는 `:not([k='menubar-item'])` 로 예전 모양 그대로
뒀습니다. 다이얼로그를 이번 변경에 딸려 보내지 않기 위해서입니다.

### 검증

`document-bar.browser.test.tsx` 에 구조 검사 하나를 더했습니다 — 메뉴가
`[k="menubar"]` 인지, 항목 다섯이 `menubar-item` 인지(다이얼로그를 여는
`Documents…` 포함), 테두리·배경이 없고 높이는 툴바 컨트롤과 같은지.
되돌리면(평범한 `<span>`/`<button>`) 실패하는 것을 확인했습니다.

동작 검사는 **원래 있던 것들이 그대로 지킵니다** — 저장 왕복, ⌘S, 다른 이름으로
저장, 목록에서 열기. `data-part` 를 그대로 두었기 때문에 한 줄도 고치지
않았습니다. 이게 이 변경을 이 모양으로 한 이유이기도 합니다.

빌드한 앱을 헤드리스 Chromium 으로 직접 몰아 메뉴 구조·hover·높이(26px, 툴바와
동일)·라이트/다크·문서 목록 열기를 확인했습니다. 콘솔 에러 0건.
