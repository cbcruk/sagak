...
- **진짜 사용자 문서(OPFS)로는 못 돌렸습니다.** 브라우저가 필요합니다
- **jsdom 이지 브라우저가 아닙니다** — `style` 파싱·직렬화가 다를 수 있습니다
- **붙여넣기 표본은 본떠 쓴 것**이지 실제 클립보드 덤프가 아닙니다
...
§2 가 이 단계입니다. 남은 것: 실제 OPFS 문서로 재확인, 붙여넣기 실물 표본, 마크 합치기
(`textStyle`) 여부, 인용·코드블록을 스키마에 넣을지.
...
- **인용·코드블록을 스키마에 넣을 것인가.** 지금 툴바에 없지만 붙여넣기로는 들어옵니다.
  안 넣으면 문단으로 풀립니다
- **값 붙는 마크 여섯을 `textStyle` 하나로 합칠 것인가.** 안 합치면 붙여넣기에서 `<span>`
  이 겹겹이 쌓입니다
