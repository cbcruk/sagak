...
- **저장물 오류를 어디서 받을 것인가** — `Node.fromJSON` 이 던집니다 (§8-2). 지금은
  `document-store` 의 `load()` 가 그대로 흘려보내고, 받아서 화면에 알리는 자리가 없습니다
- **문서 이름의 확장자** — 저장물은 JSON 인데 이름은 `메모.html` 입니다. 이름은 사용자가
  적는 것이고 내보내기는 HTML 이라 어긋난 채로 둘 수도 있습니다
- **`li > p` 여백 CSS**
