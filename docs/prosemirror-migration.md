...
| `toolbar-choice.ts` | **없어질 가능성** — 줄간격도 자간도 이제 문단 속성입니다(§15). 둘 다 읽을 수 있어졌습니다 |
...
### 닫음 — 자간을 문단 속성으로 되돌렸습니다

예전에는 걸친 블록의 `style.letterSpacing` 을 박았지만 모델에서 자간은 인라인
마크가 됐습니다. 문단 일부만 골라 줄 수 있게 된 대신, 문단 전체에 주려면 전체를
골라야 했습니다. **바뀐 것이 나은지는 그때 안 정했습니다.**

§15 에서 재고 정했습니다 — **문단 속성으로 되돌립니다.**
...
비워 둡니다. (§14 에서 돌아왔습니다. 다시 보니 **가드가 있던 자리도
틀렸습니다.**)
...
## 13. 뷰 엔진을 걷을 것인가 — 재고 판단합니다

§12 를 시작할 때의 말은 이랬습니다.

> 에디터 바깥 UI 와의 대화 — 결국 이 부분은 빌트인으로 마이그레이션 하겠습니다.
> **이제 굳이 svelte 같은 view 엔진을 사용할 이유가 사라졌습니다.**

그때는 "빌트인화 먼저, 판단은 나중" 으로 미뤘습니다. UI 층이 실제로 얇아진
뒤에 재는 것이 맞기 때문입니다. 이제 쟀습니다.

### 13-1. 얼마를 치르고 있는가

번들에서 **소스맵으로 생성 바이트를 귀속**시켰습니다 (원본 크기가 아니라
minify 된 결과 기준).

| | |
| --- | --- |
| prosemirror | 225KB (57.0%) |
| sagak-core | 57KB (14.4%) |
| sagak-ui | 51KB (13.1%) |
| **svelte 런타임** | **43KB (11.0%)** |
| lucide | 11KB (2.7%) |
| 기타 | 7KB (1.8%) |

43KB 입니다. 공짜는 아니지만 편집기 본체의 5분의 1입니다.

### 13-2. 그 대가로 무엇을 하고 있는가

컴포넌트 21개, 3068줄. `$state` 선언 34개를 성격별로 갈랐습니다.

| | 개수 |
| --- | --- |
| **UI 로컬 상태** (다이얼로그 입력칸, 열림/닫힘, 확인 중, 미리보기) | **28** |
| 코어 상태를 비추는 것 (`subscribe` 로 받아 그리기만) | 6 |

가설이 **거꾸로였습니다.** "상태 동기화가 사소해졌으니 엔진이 필요 없다" 고
봤는데, 엔진이 하는 일은 애초에 문서 상태 동기화가 아니었습니다. 그건 이제
줄 세 줄입니다 —
...
엔진이 하는 일은 **UI 로컬 상태와 DOM 사이의 배선**이고, 그 일은 §12 로
줄어들지 않았습니다. 비율로는 오히려 늘었습니다 — 코어 쪽 배선이 사라졌으니까요.

가장 무거운 컴포넌트가 그것을 보여 줍니다.

| | 코어 연결 | 로컬 상태 |
| --- | --- | --- |
| `ImageDialog` | 0 | **11** |
| `FindReplaceDialog` | 0 | 6 |
| `MoreMenu` | 1 | 3 |
| `ColorPicker` | 0 | 3 |

`ImageDialog` 는 코어와의 연결이 **하나도 없습니다.** 열 한 개 상태가 서로
물려 있는 폼일 뿐입니다. 그 파일의 주석이 옮길 때 이미 적어 뒀습니다 —
"nanotags 로 했다면 이 아홉을 `paint()` 하나에 손으로 엮어야 했습니다."

마크업 쪽도 마찬가지입니다: `{#if}` 30, `{#each}` 20, `bind:` 30.

### 13-3. 판단 — 남깁니다

걷으면 43KB 를 아끼고 **3068줄을 손으로 다시 씁니다.** 그 3068줄의 대부분은
문서와 무관한 폼과 메뉴입니다 — 엔진이 가장 잘하는 일이고, 이 저장소가 이미
한 번 값을 확인한 일입니다.

부분 제거도 셈이 안 맞습니다. 상태가 0~1개인 가벼운 것들(`HistoryButtons`
52줄, `ListButtons` 83줄, `AlignmentButtons` 61줄)을 손으로 바꿔도 다이얼로그
여섯 때문에 엔진은 그대로 남습니다. **런타임은 한 번 들어오면 반값이 없습니다.**

§12 가 바꾼 것은 뷰 엔진의 필요가 아니라 **경계의 위치**입니다. 예전에는
UI 가 이벤트 이름으로 코어와 대화하며 코어 상태의 사본을 들고 있었고, 지금은
모듈을 부르고 받은 값을 그립니다. 그 결과 UI 는 **더 UI 다워졌습니다** — 그리고
UI 다운 일에는 뷰 엔진이 맞습니다.

다시 볼 조건을 적어 둡니다. 다음 중 하나가 참이 되면 이 판단을 재검토합니다.

- 다이얼로그가 웹 컴포넌트나 다른 배포 형태로 빠져 이 저장소에서 사라짐
- 번들 예산이 생겨 43KB 가 실제 제약이 됨
- 로컬 상태가 지금의 절반 아래로 줄어듦 (지금 28개)

## 14. 이미지 크기 상한 — 가드가 있던 자리가 틀렸습니다

§11-2 에서 "보안이 아니라 UX 가드라 비워 둔다" 고 적어 둔 미결입니다. 되살리려고
옛 코드를 열어 보니, 그것이 막던 것은 **선언된 크기뿐**이었습니다.

```ts
if (data.width) {
  const widthPx = parseInt(data.width)
  if (!isNaN(widthPx) && widthPx > maxWidth) return false   // 선언이 있을 때만
}
```

붙여넣기·끌어다 놓기로 들어오는 사진에는 폭이 안 적혀 있습니다. 그래서 실제로
가장 흔한 경로가 **무방비**였습니다. 쟀습니다 —

```
600px 편집 영역에 2000px 사진 (폭 선언 없음)
  이미지 폭         2000
  영역 scrollWidth  2010     ← 가로 스크롤이 생깁니다
```

그리고 스타일시트에 `img` 규칙이 **하나도 없었습니다.**

### 14-1. 둘로 나눴습니다

§10 의 규칙 그대로입니다 — **구조·의미는 모델, 생김새는 스타일시트.**

| | 어디가 | 무엇을 |
| --- | --- | --- |
| 화면 | `img { max-width: 100%; height: auto }` | 선언이 있든 없든 편집 영역을 안 넘습니다 |
| 문서 | `insertImage` · `updateImage` | 말이 안 되는 값이 문서에 안 들어갑니다 |

`max-width` 는 인라인 `width` 보다 **나중에 적용**되므로, 문서가 `5000px` 라고
말해도 화면은 안 넘칩니다. 문서의 값은 그대로 둡니다 — 화면에 어떻게 그릴지와
문서가 무엇인지는 다른 이야기입니다.

`height: auto` 는 인라인 `height` 가 있으면 그쪽이 이깁니다. 사용자가 높이를
정한 이미지는 그대로 두고, 안 정한 것만 비율을 지킵니다.

### 14-2. 모델 쪽에서 고친 것 셋

**고칠 때도 막습니다.** 옛 판은 넣을 때만 봤습니다 — 넣고 나서 고치면 그만이라
상한이 "문서가 지키는 성질" 이 아니었습니다.

**퍼센트는 상한과 무관합니다.** `parseInt('50%')` 는 `50` 이라 옛 판은 이것을
50px 로 읽었습니다. 부모에 대한 비율은 절대 크기가 아닙니다.

**0 과 음수도 막습니다.** 상한만 보고 하한을 안 봤습니다.

### 14-3. 그리고 조용히 실패하지 않게

상한을 되살리면 예전 문제가 그대로 돌아옵니다 — 다이얼로그에서 `5000px` 를
넣고 Insert 를 누르면 **다이얼로그가 닫히고 아무 일도 안 일어납니다.** 적용이
닫은 다음 프레임에 일어나기 때문입니다. §11-2 가 "UX 가드" 라며 비워 둔 이유가
아마 이것이었을 것입니다.

두 자리를 막았습니다.

- **다이얼로그** — 거절될 값이면 Insert 를 못 누르게 하고 이유를 보여 줍니다
  (`Width exceeds the 1920px limit.`)
- **끌어서 조절** — 상한 안에서만 끌립니다. 안 그러면 손을 뗄 때 트랜잭션이
  조용히 거절되고, 화면에는 커진 채로 보이다가 다음 렌더에 되돌아갑니다

```
검사   모델 10개 · 조절 1개 · 다이얼로그 2개 신설
```

## 15. 자간 — 툴바에서 옆자리인데 규칙이 달랐습니다

§10 이 "그 판단은 쓰는 사람의 것" 이라며 남겨 둔 미결입니다. 정하기 전에
쟀습니다.

```
<p>안녕하세요 반갑습니다</p> 에 캐럿만 두고 골랐을 때

  줄 간격 2   →  <p style="line-height: 2;">안녕하세요 반갑습니다</p>
  자간 0.2em  →  <p>안녕하세요 반갑습니다</p>          ← 아무 일도 안 일어남
```

둘은 툴바의 같은 하위 목록에 나란히 있습니다. 그런데 줄 간격은 **문단 속성**
이고 자간은 **인라인 마크**였습니다. 캐럿만 있을 때 마크는 stored mark 가 되어
*다음에 치는 글자부터* 적용됩니다 — 셀렉트는 멀쩡히 값을 보여 주는데 골라도
화면이 안 바뀌는, **겉보기엔 되는 것 같은데 안 되는** 조합입니다.

### 15-1. 정한 것 — 문단 속성

줄 간격과 같은 자리로 옮겼습니다. `BLOCK_ATTRS` 에 `letterSpacing` 이 들어가고
`<p style="line-height: 2; letter-spacing: 0.2em">` 처럼 나란히 붙습니다.

**잃는 것을 적어 둡니다** — 문단 일부만 다른 자간은 이제 못 줍니다. 워드나 구글
독스에서 자간은 문자 서식이므로 일반적인 선택은 아닙니다. 다만 이 제품의 자간은
**배수 프리셋 여섯 개**이고 줄 간격 옆에 놓인 문단 꾸미기 도구입니다. 도구가
놓인 자리에 맞췄습니다.

부수적으로 저장물이 가벼워집니다 — `<span>` 이 안 생깁니다.

### 15-2. 예전 저장물을 읽어 줍니다

마크가 스키마에서 없어지면 그 마크가 든 문서는 `Node.fromJSON` 이 던집니다.

```
There is no mark type letterSpacing in this schema
```

**깨진 저장물이 아니라 우리가 깨뜨린 것**입니다. §9 의 "깨진 저장물은 던지지
않고 담는다" 는 열다 실패한 경우의 규약이고, 이건 그 앞 단계 — 애초에 실패하지
않아야 합니다.

`fromJSON` 이 옛 모양을 옮깁니다.

| 옛 저장물 | 어떻게 |
| --- | --- |
| 문단의 글이 전부 같은 자간 | 문단 속성으로 **올립니다** |
| 문단 일부만 자간 | 자간은 잃고 **글은 남깁니다** |

뒤엣것을 담을 자리가 새 모델에 없습니다. 던지는 것보다 자간을 잃는 쪽이
낫습니다 — 글이 남습니다. 툴바가 만들던 모양(고른 범위를 통째로 감쌈)은 대개
앞엣것에 해당합니다.

```
검사   모델 8개 신설 (먹이기 5 · 옛 저장물 읽기 3)
```
